...
class DebugDrawer {
  ...
  constructor(scene: any, world: any, options: Record<string, any> = {}) {
    this.scene = scene;
    this.world = world;
    ...
    const drawOnTop =
      this.debugDrawMode & AmmoDebugConstants.DrawOnTop || false;
    ...
    this.geometry.setAttribute(
      "position",
      new BufferAttribute(vertices, 3).setUsage(StaticDrawUsage)
    );
    this.geometry.setAttribute(
      "color",
      new BufferAttribute(colors, 3).setUsage(StaticDrawUsage)
    );
